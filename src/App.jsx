...
function guestInitials(name) {
  const words = String(name)
    ...
    .split(/\s+/)
    .filter(Boolean);
  return `${words[0]?.[0] || ""}${words[1]?.[0] || ""}`.toUpperCase() || "GH";
}
          ...
          <div className="arrival-list">
            {[...arrivals, ...departures, ...inHouse].slice(0, 4).map((reservation) => (
              <button className="arrival-row" key={reservation.recordKey || reservation.id} onClick={() => onOpenReservation(reservation)}>
                <span className={`arrival-avatar ${reservation.vip ? "vip" : ""}`}>{guestInitials(reservation.guest)}</span>
...
function ReservationCard({ reservation, onClick }) {
  return (
    <button className="reservation-card" onClick={onClick}>
      <div className="reservation-card-top"><span className={`guest-avatar ${reservation.vip ? "vip" : ""}`}>{guestInitials(reservation.guest)}</span><span><strong>{reservation.guest}</strong><small>{reservation.id}</small></span><MoreVertical size={17} /></div>
...
function GlobalSearchOverlay({ query, tab, onTab, onClose, onOpenReservation, reservations }) {
  ...
  return <div className="search-overlay"><button className="search-overlay-scrim" aria-label="Close search" onClick={onClose} /><section className="search-popover"><div className="search-popover-head"><div><Search size={17} /><strong>{query ? `Results for "${query}"` : "Search PMS records"}</strong></div><button onClick={onClose} aria-label="Close search"><X size={17} /></button></div><Tabs activeKey={tab} className="search-tabs" items={Object.keys(items).map((label) => ({ key: label, label: <span>{label}<b>{items[label].length}</b></span> }))} onChange={onTab} size="small" /><div className="search-result-list">{items[tab].map((record) => <button key={`${tab}-${record.recordKey || record.id}`} onClick={() => onOpenReservation(record)}><span className="guest-avatar small">{guestInitials(record.guest)}</span><span><strong>{record.guest}</strong><small>{record.id} - {record.roomType}</small></span><span className="search-stay">{record.arrival} to {record.departure}<small>{record.room} - {record.status}</small></span><ChevronRight size={16} /></button>)}</div><button className="search-view-all">View all {tab.toLowerCase()}<ChevronRight size={16} /></button></section></div>;
}
    ...
    <Drawer className="reservation-summary-drawer" closable={false} onClose={onClose} open={open} placement="right" size={456}>
      <div className="reservation-summary-shell">
        ...
        <div className="reservation-summary-guest">
          <span className={`guest-avatar large ${reservation.vip ? "vip" : ""}`}>{guestInitials(reservation.guest)}</span>
          <div><strong>{reservation.guest}</strong><small>{reservation.id}</small></div>
...
function ReservationWorkspace({ open, reservation, tab, nestedDrawer, onBack, onClose, onTab, onNestedDrawer }) {
  if (!reservation) return null;
  const summary = <div className="reservation-workspace-summary"><button onClick={onBack} className="back-workspace" aria-label="Back to reservation detail"><ChevronLeft size={18} /></button><span className="guest-avatar large">{guestInitials(reservation.guest)}</span><div><strong>{reservation.guest}</strong><small>{reservation.id} - {reservation.status}</small></div><div><span>Arrival</span><b>{reservation.arrival}</b></div><div><span>Departure</span><b>{reservation.departure}</b></div><div><span>Nights</span><b>{reservation.nights}</b></div><div><span>Room / Room Type</span><b>{reservation.room} / {reservation.roomType}</b></div><div className="workspace-summary-actions"><Button onClick={onBack} size="small">Reservation Detail</Button><Button onClick={onClose} size="small">Close</Button></div></div>;
