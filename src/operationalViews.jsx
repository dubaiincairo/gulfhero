...
export function HousekeepingView({ roomRows }) {
  ...
  const [notice, setNotice] = useState("Housekeeping is a local operational review for the current business date.");
  ...
  const saveRoomReview = () => {
    setHouseRows((current) => current.map((row) => row.number === roomReview.number ? { ...row, condition: reviewCondition, assigned: reviewAssignee, note: `${reviewCondition} locally by ${reviewAssignee}` } : row));
    setRoomReview(null);
    setNotice(`Room ${roomReview.number} updated for local housekeeping review.`);
  };
  const applyBlock = () => {
    ...
    setBlockRows((current) => [{ key, room: blockDraft.room, roomType: room?.type || "Selected room", from: blockDraft.from, to: blockDraft.to, blockedOn: "Created locally", blockedBy: "Housekeeping Manager", reason: blockDraft.reason.trim() }, ...current]);
    setHouseRows((current) => current.map((row) => row.number === blockDraft.room ? { ...row, status: "Blocked", condition: "Maintenance", assigned: "Engineering", note: `Local block: ${blockDraft.reason.trim()}` } : row));
    setBlockModalOpen(false);
    setBlockDraft({ room: "", from: "09/07/2026", to: "10/07/2026", reason: "" });
    setTab("Maintenance Block");
    setNotice(`Room ${room?.number || blockDraft.room} was added to the local maintenance block list.`);
  };
  const saveTask = () => {
    if (!taskReady) return;
    const order = `WO-${String(219 + taskRows.length).padStart(3, "0")}`;
    setTaskRows((current) => [{ key: `task-${Date.now()}`, order, room: taskDraft.room, folio: taskDraft.folio || "—", category: taskDraft.category, description: taskDraft.description.trim(), priority: taskDraft.priority, assignee: taskDraft.assignee, entered: "09 Jul 2026, local", updated: "Just now", deadline: taskDraft.due || "No deadline", status: "Open" }, ...current]);
    setTaskModalOpen(false);
    setTaskDraft({ room: "", category: "", priority: "", description: "", assignee: "", folio: "", due: "", blockFrom: "", blockTo: "" });
    setTab("Work Order / Task");
    setNotice(`${order} was saved as a local housekeeping task.`);
  };

  const house = <><div className="housekeeping-filter-bar"><Input allowClear aria-label="Search house status" onChange={(event) => setRoomSearch(event.target.value)} placeholder="Search room or room type" prefix={<Search size={15} />} value={roomSearch} /><Select aria-label="Room status filter" onChange={setRoomCondition} options={[{ value: "All room statuses", label: "All room statuses" }, { value: "Clean", label: "Clean" }, { value: "Dirty", label: "Dirty" }, { value: "Inspected", label: "Inspected" }, { value: "Maintenance", label: "Maintenance" }]} size="small" value={roomCondition} /><Button onClick={() => { setRoomSearch(""); setRoomCondition("All room statuses"); setNotice("House Status filters were cleared locally."); }} size="small">Clear</Button></div><div className="house-status-grid"><WorkspaceTable columns={[{ title: "Room Type / Room", dataIndex: "number", width: 205, render: (value, row) => <span className="house-room-cell"><b>{row.type}</b><small>Room {value}</small></span> }, { title: "Pax", dataIndex: "pax", width: 54, align: "center" }, { title: "House Status", dataIndex: "status", width: 108, render: (value) => <StatusTag value={value} /> }, { title: "Assigned To", dataIndex: "assigned", width: 112 }, { title: "Room Status", dataIndex: "condition", width: 106, render: (value) => <StatusTag value={value} /> }, { title: "Arrival Time", dataIndex: "arrivalTime", width: 92 }, { title: "Arrival", dataIndex: "arrival", width: 77 }, { title: "Departure", dataIndex: "departure", width: 84 }, { title: "Nights", dataIndex: "nights", width: 58, align: "center" }, { title: "Remarks", dataIndex: "note", width: 140 }, { title: "Action", width: 92, render: (_, row) => <Button onClick={() => openRoomReview(row)} size="small">Review</Button> }]} rowKey="number" rows={filteredHouseRows} /></div></>;
  const maintenance = <><div className="housekeeping-filter-bar"><Input allowClear aria-label="Search maintenance blocks" onChange={(event) => setBlockSearch(event.target.value)} placeholder="Search room or room type" prefix={<Search size={15} />} value={blockSearch} /><Select aria-label="Maintenance room type filter" onChange={setBlockRoomType} options={[{ value: "All room types", label: "All room types" }, ...roomTypes.map((type) => ({ value: type, label: type }))]} size="small" value={blockRoomType} /><Button onClick={() => setBlockFilterOpen(true)} size="small">Search</Button><Button onClick={() => { setBlockSearch(""); setBlockRoomType("All room types"); setNotice("Maintenance Block filters were cleared locally."); }} size="small">Clear</Button></div><WorkspaceTable columns={[{ title: "Room", dataIndex: "room", width: 90 }, { title: "Block From", dataIndex: "from", width: 135 }, { title: "Block To", dataIndex: "to", width: 135 }, { title: "Blocked On", dataIndex: "blockedOn", width: 150 }, { title: "Blocked By", dataIndex: "blockedBy", width: 140 }, { title: "Reason", dataIndex: "reason" }, { title: "Action", width: 92, render: (_, row) => <Button onClick={() => setNotice(`Maintenance block for room ${row.room} selected for local review.`)} size="small">Review</Button> }]} rows={filteredBlocks} /></>;
  const tasks = <><div className="housekeeping-filter-bar task"><Input allowClear aria-label="Search work orders" onChange={(event) => setTaskSearch(event.target.value)} placeholder="Search order, room, category or description" prefix={<Search size={15} />} value={taskSearch} /><Select aria-label="Work order status filter" onChange={setTaskStatus} options={[{ value: "All statuses", label: "All statuses" }, { value: "Open", label: "Open" }, { value: "In progress", label: "In progress" }, { value: "Resolved", label: "Resolved" }]} size="small" value={taskStatus} /><Checkbox checked={showCompletedTasks} onChange={(event) => setShowCompletedTasks(event.target.checked)}>List completed tasks</Checkbox><Button onClick={() => setNotice("Work Order results refreshed locally.")} size="small">Search</Button><Button onClick={() => { setTaskSearch(""); setTaskStatus("All statuses"); setShowCompletedTasks(false); setNotice("Work Order filters were cleared locally."); }} size="small">Clear</Button></div><WorkspaceTable columns={[{ title: "Order #", dataIndex: "order", width: 84 }, { title: "Unit/Room", dataIndex: "room", width: 76 }, { title: "Reservation/Folio", dataIndex: "folio", width: 112 }, { title: "Category", dataIndex: "category", width: 105 }, { title: "Description", dataIndex: "description" }, { title: "Priority", dataIndex: "priority", width: 88, render: (value) => <InlineStatus tone={value === "High" ? "danger" : "muted"}>{value}</InlineStatus> }, { title: "Assign To", dataIndex: "assignee", width: 108 }, { title: "Entered On", dataIndex: "entered", width: 125 }, { title: "Updated", dataIndex: "updated", width: 110 }, { title: "Deadline", dataIndex: "deadline", width: 100 }, { title: "Status", dataIndex: "status", width: 106, render: (value) => <StatusTag value={value} /> }, { title: "Action", width: 96, render: (_, row) => <Button disabled={row.status === "Resolved"} onClick={() => { setTaskRows((current) => current.map((task) => task.key === row.key ? { ...task, status: "Resolved", updated: "Just now" } : task)); setNotice(`${row.order} marked resolved locally.`); }} size="small">{row.status === "Resolved" ? "Resolved" : "Resolve"}</Button> }]} rows={filteredTasks} /></>;
  const tabs = { "House Status": house, "Maintenance Block": maintenance, "Work Order / Task": tasks };
  const toolbar = tab === "House Status" ? <><Button onClick={() => setNotice("House Status print preview prepared locally.")} size="small">Print</Button><Button icon={<Download size={14} />} onClick={() => setNotice("House Status export preview prepared locally.")} size="small">Export</Button><Button onClick={() => setSettingsOpen(true)} size="small">Settings</Button><Button onClick={() => setAuditOpen(true)} size="small">Audit Trail</Button></> : tab === "Maintenance Block" ? <><Button className="primary-command" icon={<Plus size={14} />} onClick={() => setBlockModalOpen(true)} size="small">Block Room</Button><Button icon={<Download size={14} />} onClick={() => setNotice("Maintenance Block export preview prepared locally.")} size="small">Export</Button></> : <><Button className="primary-command" icon={<Plus size={14} />} onClick={() => setTaskModalOpen(true)} size="small">Add Task</Button><Button icon={<Download size={14} />} onClick={() => setNotice("Work Order export preview prepared locally.")} size="small">Export</Button></>;

  return <section className="housekeeping-view"><div className="view-tabs-toolbar"><Tabs activeKey={tab} className="pms-tabs" items={Object.keys(tabs).map((label) => ({ key: label, label }))} onChange={setTab} size="small" /><div className="toolbar-actions">{toolbar}</div></div><div className="house-summary"><MiniMetric label="Clean" value="49" detail="Ready for arrival" tone="green" /><MiniMetric label="Dirty" value="05" detail="Needs attention" tone="yellow" /><MiniMetric label="Inspected" value="18" detail="Supervisor cleared" tone="blue" /><MiniMetric label="Maintenance" value="04" detail="Blocked rooms" tone="purple" /></div>{tabs[tab]}<div className="surface-status"><ClipboardCheck size={14} />{notice}</div><Drawer className="housekeeping-review-drawer" onClose={() => setRoomReview(null)} open={Boolean(roomReview)} placement="right" size={420} title={roomReview ? `Room ${roomReview.number} Review` : "Room Review"}><div className="drawer-form"><div className="housekeeping-local-note">This review updates only the local desktop prototype state.</div><label>Room Status<Select onChange={setReviewCondition} options={[{ value: "Clean", label: "Clean" }, { value: "Dirty", label: "Dirty" }, { value: "Inspected", label: "Inspected" }, { value: "Maintenance", label: "Maintenance" }]} value={reviewCondition} /></label><label>Assigned To<Select onChange={setReviewAssignee} options={[{ value: "Housekeeping", label: "Housekeeping" }, { value: "Engineering", label: "Engineering" }, { value: "Front Office", label: "Front Office" }]} value={reviewAssignee} /></label><div className="drawer-form-actions"><Button onClick={() => setRoomReview(null)}>Cancel</Button><Button className="primary-command" onClick={saveRoomReview}>Save Local Review</Button></div></div></Drawer><Drawer className="housekeeping-settings-drawer" onClose={() => setSettingsOpen(false)} open={settingsOpen} placement="right" size={390} title="House Status Settings"><div className="drawer-form"><div className="housekeeping-local-note">Display preferences are a local review state only.</div><label>View Mode<Select defaultValue="Room type grouped" options={[{ value: "Room type grouped", label: "Room type grouped" }, { value: "Room number", label: "Room number" }]} /></label><label>Show remarks<Checkbox defaultChecked>Show operational remarks</Checkbox></label><div className="drawer-form-actions"><Button onClick={() => setSettingsOpen(false)}>Close</Button><Button className="primary-command" onClick={() => { setSettingsOpen(false); setNotice("House Status display settings saved locally."); }}>Save Local Settings</Button></div></div></Drawer><Drawer className="housekeeping-audit-drawer" onClose={() => setAuditOpen(false)} open={auditOpen} placement="right" size={400} title="Housekeeping Audit Trail"><div className="activity-timeline"><div><i className="green" /><span><b>Room 203 inspection queued</b><small>09 Jul 2026, 09:10 AM · Local review fixture</small></span></div><div><i className="yellow" /><span><b>Room 512 maintenance block reviewed</b><small>09 Jul 2026, 08:45 AM · Local review fixture</small></span></div><div><i className="blue" /><span><b>House Status view opened</b><small>09 Jul 2026, 08:30 AM · No external update</small></span></div></div><div className="drawer-form-actions"><Button onClick={() => setAuditOpen(false)}>Close</Button></div></Drawer><Drawer className="maintenance-search-drawer" onClose={() => setBlockFilterOpen(false)} open={blockFilterOpen} placement="right" size={400} title="Search Maintenance Block"><div className="drawer-form"><div className="housekeeping-local-note">Filters apply only to this local maintenance list.</div><label>Block Date<Input readOnly value="09/07/2026" /></label><label>Room Type<Select onChange={setBlockRoomType} options={[{ value: "All room types", label: "All room types" }, ...roomTypes.map((type) => ({ value: type, label: type }))]} value={blockRoomType} /></label><label className="drawer-filter-toggle"><Checkbox>Unblock Room</Checkbox></label><div className="drawer-form-actions"><Button onClick={() => { setBlockRoomType("All room types"); setBlockFilterOpen(false); }}>Clear</Button><Button className="primary-command" onClick={() => { setBlockFilterOpen(false); setNotice("Maintenance Block search applied locally."); }}>Search</Button></div></div></Drawer><Modal className="housekeeping-modal" footer={null} onCancel={() => setBlockModalOpen(false)} open={blockModalOpen} title="Block Room"><div className="housekeeping-local-note">Creates a temporary local maintenance block only. It does not alter property operations.</div><div className="housekeeping-modal-form"><label>Block From<Input aria-label="Block From" onChange={(event) => setBlockDraft((current) => ({ ...current, from: event.target.value }))} value={blockDraft.from} /></label><label>Block To<Input aria-label="Block To" onChange={(event) => setBlockDraft((current) => ({ ...current, to: event.target.value }))} value={blockDraft.to} /></label><label>Room Type<Select aria-label="Block Room Type" defaultValue="Select room type" options={[{ value: "Select room type", label: "Select room type" }, ...roomTypes.map((type) => ({ value: type, label: type }))]} /></label><label>Room List<Select aria-label="Block Room List" onChange={(value) => setBlockDraft((current) => ({ ...current, room: value }))} options={[{ value: "", label: "Select room" }, ...houseRows.map((row) => ({ value: row.number, label: `${row.number} · ${row.type}` }))]} value={blockDraft.room} /></label><label>Status<Select aria-label="Block Status" defaultValue="Blocked" options={[{ value: "Blocked", label: "Blocked" }, { value: "Maintenance", label: "Maintenance" }]} /></label><label>Reason<Input.TextArea aria-label="Block Reason" onChange={(event) => setBlockDraft((current) => ({ ...current, reason: event.target.value }))} placeholder="Required reason" rows={2} value={blockDraft.reason} /></label></div><div className="drawer-form-actions"><Button onClick={() => setBlockDraft({ room: "", from: "09/07/2026", to: "10/07/2026", reason: "" })}>Clear</Button><Button className="primary-command" disabled={!blockDraft.room || !blockDraft.reason.trim()} onClick={applyBlock}>Apply Local Block</Button></div></Modal><Modal className="housekeeping-modal task-modal" footer={null} onCancel={() => setTaskModalOpen(false)} open={taskModalOpen} title="Add Task"><div className="housekeeping-local-note">Creates a temporary local work order only. No external assignment is sent.</div><div className="housekeeping-modal-form task"><label>Unit/Room<Select aria-label="Task Room" onChange={(value) => setTaskDraft((current) => ({ ...current, room: value }))} options={[{ value: "", label: "Select room" }, ...houseRows.map((row) => ({ value: row.number, label: `${row.number} · ${row.type}` }))]} value={taskDraft.room} /></label><label>Category<Select aria-label="Task Category" onChange={(value) => setTaskDraft((current) => ({ ...current, category: value }))} options={[{ value: "", label: "Select category" }, { value: "Inspection", label: "Inspection" }, { value: "Maintenance", label: "Maintenance" }, { value: "Cleaning", label: "Cleaning" }]} value={taskDraft.category} /></label><label>Priority<Select aria-label="Task Priority" onChange={(value) => setTaskDraft((current) => ({ ...current, priority: value }))} options={[{ value: "", label: "Select priority" }, { value: "High", label: "High" }, { value: "Normal", label: "Normal" }, { value: "Low", label: "Low" }]} value={taskDraft.priority} /></label><label>Assign To<Select aria-label="Task Assignee" onChange={(value) => setTaskDraft((current) => ({ ...current, assignee: value }))} options={[{ value: "", label: "Select assignee" }, { value: "Housekeeping", label: "Housekeeping" }, { value: "Engineering", label: "Engineering" }, { value: "Front Office", label: "Front Office" }]} value={taskDraft.assignee} /></label><label className="task-description">Description<Input.TextArea aria-label="Task Description" onChange={(event) => setTaskDraft((current) => ({ ...current, description: event.target.value }))} placeholder="Required task description" rows={2} value={taskDraft.description} /></label><label>Reservation/Folio<Input aria-label="Task Folio" onChange={(event) => setTaskDraft((current) => ({ ...current, folio: event.target.value }))} placeholder="Optional folio" value={taskDraft.folio} /></label><label>Due Date / Time<Input aria-label="Task Due Date" onChange={(event) => setTaskDraft((current) => ({ ...current, due: event.target.value }))} placeholder="Optional date and time" value={taskDraft.due} /></label><label>Block From<Input aria-label="Task Block From" onChange={(event) => setTaskDraft((current) => ({ ...current, blockFrom: event.target.value }))} placeholder="Optional block date" value={taskDraft.blockFrom} /></label><label>Block To<Input aria-label="Task Block To" onChange={(event) => setTaskDraft((current) => ({ ...current, blockTo: event.target.value }))} placeholder="Optional block date" value={taskDraft.blockTo} /></label></div><div className="drawer-form-actions"><Button onClick={() => setTaskModalOpen(false)}>Cancel</Button><Button className="primary-command" disabled={!taskReady} onClick={saveTask}>Save Local Task</Button></div></Modal></section>;
}
